#!/bin/bash
# Lambda Cloud間直接データ転送スクリプト（より効率的）
...
# Step 4: 転送実行確認
echo ""
read -p "直接転送を実行しますか？ (y/N): " confirm
if [[ ! $confirm =~ ^[Yy]$ ]]; then
    echo "❌ 転送をキャンセルしました"
    exit 1
fi

# Step 5: 転送先から転送元へ直接rsync実行
echo ""
echo "🔄 Step 5: インスタンス間直接rsync転送開始"
...
# Step 8: セキュリティ: 一時SSHキー削除
echo ""
echo "🔒 Step 8: セキュリティクリーンアップ"
