...
class QFormerSegmentationBridge(nn.Module):
    ...
    def forward(
        ...
        try:
            # Web調査結果: BLIP-2のテキストプロンプト明示的指定でNoneTypeエラー解決
            # ダミーテキストプロンプトを作成（BLIP-2ベストプラクティス）
            dummy_text_prompts = [" "] * batch_size  # 空文字列プロンプト
            
            qformer_outputs = self.qformer(
                encoder_hidden_states=encoder_hidden_states,
                encoder_attention_mask=attention_mask,
                text_input=dummy_text_prompts,  # Web調査結果: テキストプロンプト追加
                output_attentions=False,
                return_dict=True
            )
            print(f"  ✅ 公式Q-Former成功: テキストプロンプト修正で解決")
