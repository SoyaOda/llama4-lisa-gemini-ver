...
実行方法:
# 最小時間テスト（10ステップ）- OOM修正版
ssh -i ~/.ssh/lambda_cloud_key ubuntu@192.222.55.242 "cd /lambda/nfs/llama4-lisa-project-fs-central-texas/code/LISA-Gemma-Linux && source ../../venvs/lisa_gemma_venv/bin/activate && export CUDA_VISIBLE_DEVICES=0,1 && export PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:256,expandable_segments:True && PYTHONUNBUFFERED=1 python -u train_phase3b_qformer_bridge.py --exp_name quick_test --steps_per_epoch 10 --epochs 1 2>&1"

# 通常訓練（実際のデータで損失減少観察）- OOM修正版
...
# Step 2.1: GPU RAM分散利用のための追加設定（参考ファイル準拠）
os.environ['PYTORCH_CUDA_ALLOC_CONF'] = 'max_split_size_mb:256,expandable_segments:True'  # GPU RAM最適化
...
if torch.cuda.is_available():
    torch.cuda.set_per_process_memory_fraction(0.9)  # 90%使用許可（参考ファイル準拠）
    print("🔧 CUDA memory fraction set to 0.9 (90% usage, reference files compliant)")
    ...
    try:
        ...
        if TRANSFORMERS_AVAILABLE:
            ...
            try:
                ...
                # 🔥 Model Parallelism復活: 2x H100分散利用（17Bモデル必須）
                logger.info("🔥 Model Parallelism復活: 2x H100分散利用（17Bモデル対応）")
                device_count = torch.cuda.device_count()
                logger.info(f"検出されたGPU数: {device_count}")
                
                if device_count >= 2:
                    # 2x H100対応: 参考ファイル準拠の安全設定
                    # 🚨 最終修正: disk完全回避のため制限大幅緩和
                    max_memory_per_gpu = "75GB"  # 70GB → 75GB（H100最大限活用）
                    ...
                    max_memory_dict = {
                        ...
                        "cpu": "50GB"  # CPU大幅増加でdisk完全回避（diskキー除外）
                    }
                    
                    logger.info(f"✓ Model Parallelism: 各GPU {max_memory_per_gpu} 制限（バランス設定）")
                    
                    # CPU offload設定（メモリ効率化）
                    offload_folder = "/tmp/llama4_offload"
                    
                else:
                    raise RuntimeError("❌ 2x H100環境が必要です（17Bモデルは単一GPU不可）")
                ...
                # 🚨 修正: "auto"を削除してシンプルなdevice_map戦略
                # max_memoryで制御しつつ、重要コンポーネントのみ明示指定
                load_kwargs_device_map = "auto"  # 基本は auto を使用
                
                load_kwargs = {
                    "torch_dtype": torch.bfloat16,
                    "device_map": load_kwargs_device_map,  # シンプルなauto使用
                    "attn_implementation": "sdpa",
                    "trust_remote_code": True,
                    "low_cpu_mem_usage": True,
                    "max_memory": max_memory_dict,  # 70GB + CPU制限でdisk回避
                    "offload_state_dict": False,  # state_dictのオフロードを無効化
                    # no_split_module_classes削除: Llama4ForCausalLM未対応
                    # tie_word_embeddings削除: Llama4ForCausalLM未対応
                }
                
                logger.info(f"🔧 数値安定化設定適用: rms_norm_eps={config_linux.LAYERNORM_EPSILON}")
                
                # CPU offload設定（必要に応じて）
                # 🔧 方針B: meta tensor device mismatch回避のため、offload_folderを無効化
                # if offload_folder and device_count >= 2:
                #     import os
                #     os.makedirs(offload_folder, exist_ok=True)
                #     load_kwargs["offload_folder"] = offload_folder
                #     logger.info(f"✓ CPU offload有効: {offload_folder}")
                logger.info("🔧 CPU offload無効化: meta tensor device mismatch回避")
                ...
                if hasattr(llama4_model, 'lm_head'):
                    try:
                        ...
                        if str(lm_head_device) == 'meta' or 'disk' in str(lm_head_device):
                            ...
                            logger.info(f"  ✅ lm_head正常配置: {lm_head_device}")
...
def main():
    ...
    try:
        ...
        # 6. 結果保存 - デバッグ中は無効化
        logger.info("\n🎯 訓練完了")
        logger.info("📌 デバッグモード: 結果保存はスキップされました")
