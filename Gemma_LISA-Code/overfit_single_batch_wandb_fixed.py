...
5.1. サニティチェック1：単一バッチへの過学習（WandB統合修正版）

WandB統合により実験結果を体系的に追跡・可視化する。
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="単一バッチでの過学習テスト（WandB統合修正版）")
...
def main():
    ...
    print("5.1. サニティチェック1：単一バッチへの過学習（WandB統合修正版）")
    print("="*80)
    ...
    try:
        ...
        session_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        wandb.init(
            project="lisa-gemma-overfit-test", 
            name=f"overfit-test-{session_timestamp}",
            config={
        ...
        # WandBに分析結果をログ
        wandb.log({
            "analysis/success": analysis['success'],
            "analysis/initial_loss": analysis['initial_loss'],
            "analysis/final_loss": analysis['final_loss'],
            "analysis/reduction_ratio": analysis['reduction_ratio'],
            "analysis/seg_reduction_ratio": analysis['seg_reduction_ratio'],
            "analysis/text_reduction_ratio": analysis['text_reduction_ratio'],
            "analysis/stability_ratio": analysis['stability_ratio'],
        })
        ...
        # 損失曲線のプロット
        plot_path = os.path.join(output_dir, f"overfit_single_batch_wandb_fixed_{session_timestamp}.png")
        ...
        json_path = os.path.join(output_dir, f"overfit_single_batch_wandb_fixed_{session_timestamp}.json")
        ...
        # WandBに最終結果を記録
        wandb.summary["final_status"] = "success" if analysis['success'] else "failure"
        wandb.summary["final_loss"] = analysis['final_loss']
        wandb.summary["reduction_ratio"] = analysis['reduction_ratio']
        wandb.summary["test_iterations"] = args.iterations
